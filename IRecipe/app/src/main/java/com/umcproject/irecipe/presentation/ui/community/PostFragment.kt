...
class PostFragment(private val onCLickBackBtn: (String) -> Unit,private val index: Int,private val communityFragment: CommunityFragment) : BaseFragment<FragmentPostBinding>() {
    ...
    private lateinit var writePost: WritePost
    ...
    private fun getData() {
        ...
        if (postJson != null) {
            writePost = gson.fromJson(postJson, WritePost::class.java)
            initView(writePost)
        } else {
            Toast.makeText(context, "데이터를 가져오는 데 실패했습니다.", Toast.LENGTH_SHORT).show()
        }
    }
    private fun initView(writePost:WritePost) {
        binding.tvTitle.text = writePost.title
        binding.tvText.text = writePost.content
